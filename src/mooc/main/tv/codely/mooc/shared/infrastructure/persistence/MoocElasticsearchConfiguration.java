...
@Configuration
public class MoocElasticsearchConfiguration {
    private final Parameter               config;
    private final ResourcePatternResolver resourceResolver;

    public MoocElasticsearchConfiguration(Parameter config, ResourcePatternResolver resourceResolver) {
        this.config           = config;
    ...
    public ElasticsearchClient elasticsearchClient() throws ParameterNotExist, IOException {
        ElasticsearchClient client = new ElasticsearchClient(
            new RestHighLevelClient(
                RestClient.builder(
                    new HttpHost(
                        config.get("MOOC_ELASTICSEARCH_HOST"),
                        config.getInt("MOOC_ELASTICSEARCH_PORT"),
                        "http"
                    )
                )
            ),
            ...
            config.get("MOOC_ELASTICSEARCH_INDEX_PREFIX")
        );

        generateIndexIfNotExists(client, "mooc");
